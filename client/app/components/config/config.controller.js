...
class ConfigController {
  /* @ngInject */
  constructor($rootScope, $scope, $state, $stateParams, $window, $timeout, $q, $log, ModalService, ConfigFactory, HelperFactory, uiGridConstants, $mdDialog) {
    const vm = this;

    vm.configType = $stateParams.configType;
    ...
    vm.info = info[vm.configType];

    if (/schema|field|action|taxonomy/.test(vm.configType)) {
      vm.columnDefs.push({
        name: 'name',
        sortType: 'string',
        displayName: 'Name',
    ...
    function getResults (reset) {
      ...
      ConfigFactory.search(vm.configType, options)
        .then((items) => {
    ...
    vm.editItems = (items, type) => {
      ...
      ConfigFactory.editItems(type, items)
        .then(updateItems);
    };
    ...
    vm.deleteItems = (items, type) => {
      ...
      $mdDialog
        .show(confirm)
        .then(() => {
          ConfigFactory.deleteItems(type, items)
            .then((data) => {
              if (!data.error) {
                getResults(true);
              }
    ...
    vm.cloneItem = (items, type) => {
      ...
      ConfigFactory.cloneItem(type, items[0])
        .then((item) => {
          vm.items.push(item);
