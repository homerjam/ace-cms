...
angular
  ...
  .config(
    (
      ...
      tmhDynamicLocaleProvider.localeLocationPattern(
        'js/angular-i18n/angular-locale_{{locale}}.js'
      );
            ...
            $mdDialog.show(
              $mdDialog
                ...
                .htmlContent(
                  `
                <p>${message}</p>
                <!--<p>Please <a href="${$window.location.origin +
                  appConfig.clientBasePath +
                  appConfig.slug}/logout" target="_blank">login</a> again.</p>-->
              `
                )
                .ok('Close')
            );
  ...
  .run(
    (
      ...
      setApiToken(
        $location.search().apiToken || localStorage.getItem('apiToken')
      );

      const renewToken = async (forceRenew = false) => {
        if (!$document[0].hidden || forceRenew) {
          const apiToken = await $http.get(`${appConfig.apiUrl}/token`, {
          ...
          ConfigFactory.loadConfig().then(
            config => {
              if (config.slug !== appConfig.slug) {
                $window.location.href = `${appConfig.clientBasePath +
                  appConfig.slug}/logout`;
                return;
              }
              ...
              $rootScope.assetSlug =
                config.assets && config.assets.slug
                  ? config.assets.slug
                  : appConfig.slug;
